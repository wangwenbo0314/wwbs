...
import { NavController, NavParams, normalizeURL, ActionSheetController, LoadingController, ToastController, Platform, ViewController } from 'ionic-angular';
...
export class HeadfacePage extends BaseUI {
  ...
  takePicture(sourceType) {
    ...
    this.camera.getPicture(options).then((imagePath) => {
      ...
      if (this.paltform.is('android') && sourceType === this.camera.PictureSourceType.PHOTOLIBRARY) {
        this.filepath.resolveNativePath(imagePath)//获取Android
          .then(filePath => {
            //获取正确的路径
  ...
  copyFileToLocalDir(namePath, correctName, newFilename) {
    this.file.copyFile(namePath, correctName, cordova.file.dataDirectory, newFilename).then(success => {
  ...
  uploadImage() {
    ...
    var filname = this.userId + ".jpg";//定义上传后的文件名
    var options = {
      fileKey: "file",
      fileName: filname,
      chunkedMode: false,
      mimeType: "multipart/form-data",
      params: { 'fileName': filname, 'userid': this.userId }
    };
